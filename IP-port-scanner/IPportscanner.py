...
class ScanThread(threading.Thread):
    ...
    def run(self):
        try:
            ...
            print(colored(f"[-] Port {self.port} is closed.", "red"))
        finally:
            client.close()

# Define a function that will be used to scan the target(s) for open ports
def scan_targets(target, port_range, timeout, num_threads):
    all_open_ports = []
...
def main():
    ...
    # Prompt the user to enter the port range
    port_range = input("Enter the port range (e.g. 1-100 or all): ")
    # Prompt the user to enter the timeout value
    timeout = float(input("Enter the timeout value in seconds (default: 1.0): ") or 1.0)
    # Prompt the user to enter the number of threads
    num_threads = int(input("Enter the number of threads to use for scanning (default: 10): ") or 10)
    # Prompt the user to enter the output file name
    output_file = input("Enter the output file name (optional): ")

    # Run port scan for the specified target
    all_open_ports = scan_targets(target, port_range, timeout, num_threads)
